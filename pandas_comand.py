...
#************************************************************************************************************************************************************************************
with st.expander("**:black_square_button: Operazioni sulle stringhe di un DataFrame**"):

    st.write(
        """
        Le colonne di tipo stringa (o 'object' in pandas) hanno una serie di metodi speciali, 
        accessibili tramite l'attributo `.str`. Questi metodi permettono di eseguire operazioni
        di manipolazione del testo in modo efficiente su intere colonne.
        """
    )

    # Creiamo una copia del DataFrame originale per non modificarlo negli esempi successivi
    clienti_str = clienti.copy()

    #----------------------------------------------------------------------
    st.write("#### 1. Filtrare righe con `str.contains`")
    st.write("Filtra il DataFrame mantenendo solo le righe che contengono una specifica sottostringa in una colonna. È l'equivalente di un `LIKE` in SQL.")
    code = """
# Filtriamo i clienti che vivono a 'Roma'
...
st.dataframe(clienti_roma, use_container_width=True, hide_index=True)
    ...
    st.write("Utile per standardizzare i dati testuali e rendere i filtri più affidabili.")
    code = """
# Creiamo due nuove colonne con i nomi in minuscolo e maiuscolo
clienti_str['nome_minuscolo'] = clienti_str['nome_cliente'].str.lower()
clienti_str['citta_maiuscolo'] = clienti_str['citta'].str.upper()
st.dataframe(clienti_str[['nome_cliente', 'nome_minuscolo', 'citta', 'citta_maiuscolo']], use_container_width=True, hide_index=True)
    """
    st.code(code, language="python")
    ...
    st.write("Pulisce gli spazi vuoti (o altri caratteri specificati) all'inizio e alla fine di una stringa. Esistono anche `str.lstrip()` per pulire solo a sinistra e `str.rstrip()` solo a destra.")
    
    # Aggiungiamo uno spazio per l'esempio
    clienti_spazi = clienti.copy()
    clienti_spazi.loc[0, 'nome_cliente'] = '  Mario Rossi  '
    st.write("DataFrame con spazi aggiunti:")
    st.dataframe(clienti_spazi, use_container_width=True, hide_index=True)

    code = """
# Aggiungiamo spazi a un nome per dimostrazione: '  Mario Rossi  '
# Poi usiamo strip() per pulire la colonna
clienti_spazi['nome_pulito'] = clienti_spazi['nome_cliente'].str.strip()
st.dataframe(clienti_spazi[['nome_cliente', 'nome_pulito']], use_container_width=True, hide_index=True)
    """
    st.code(code, language="python")
    clienti_spazi['nome_pulito'] = clienti_spazi['nome_cliente'].str.strip()
    st.write("DataFrame dopo la pulizia con `strip()`:")
    st.dataframe(clienti_spazi[['nome_cliente', 'nome_pulito']], use_container_width=True, hide_index=True)
    
    #----------------------------------------------------------------------
    st.write("#### 4. Sostituire testo con `str.replace()`")
    st.write("Cerca una sottostringa e la sostituisce con un'altra.")
    code = """
# Sostituiamo 'Rossi' con 'Gialli' nella colonna 'nome_cliente'
...
st.dataframe(clienti_sostituzione, use_container_width=True, hide_index=True)
    ...
    st.write("Divide una stringa in base a un delimitatore (come uno spazio) e può creare nuove colonne. Molto utile per separare nomi e cognomi, indirizzi, ecc.")
    code = """
# Dividiamo la colonna 'nome_cliente' in 'Nome' e 'Cognome'
# expand=True dice a pandas di creare nuove colonne per ogni pezzo
...
st.dataframe(clienti_split, use_container_width=True, hide_index=True)
    ...
    #----------------------------------------------------------------------
    st.write("#### 6. Unire più colonne")
    code = """
# Uniamo la colonna 'nome_cliente' con 'citta'
clienti_unione = clienti.copy()
clienti_unione['nome_cognome_citta'] = clienti_unione['nome_cliente'] + ' ' + clienti_unione['citta']
st.dataframe(clienti_unione, use_container_width=True, hide_index=True)
    """
    st.code(code, language="python")
    ...
    #----------------------------------------------------------------------
    st.write("#### 7. Calcolare la lunghezza con `str.len()`")
    st.write("Restituisce una Serie con la lunghezza (numero di caratteri) di ogni stringa.")
    code = """
# Creiamo una nuova colonna con la lunghezza di ogni nome
clienti_len = clienti.copy()
clienti_len['lunghezza_nome'] = clienti_len['nome_cliente'].str.len()
st.dataframe(clienti_len, use_container_width=True, hide_index=True)
    """
    st.code(code, language="python")
    clienti_len = clienti.copy()
    clienti_len['lunghezza_nome'] = clienti_len['nome_cliente'].str.len()
